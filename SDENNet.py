...
layers.append(LinearLayer(X_train.shape[1], hidden_neurons_1, sigm, n_each))
layers.append(LogisticLayer())
# Add second hidden layer
# layers.append(LinearLayer(hidden_neurons_1, hidden_neurons_2, sigm, n_each))
# layers.append(LogisticLayer())
# Add output layer
layers.append(LinearLayer(hidden_neurons_2, T_train.shape[1], sigm, n_each))
...
# sigm=1,epoch=300,仿真次数=10000,threshold -> 看与自己的程序结果的差异
...
#introduction
print("SDENNet_introduction : new,single_layer,threshold,sigm=" + str(sigm) + ",neures=" + str(hidden_neurons_1) + ",total_epoch=" + str(max_nb_of_iterations) )  #**
...
x_str = 'epoch   ' + 'val_acc={:.2f},test_acc={:.2f},epochces={:.0f},sigm='.format(val_accuracy,test_accuracy,int(iteration + 1)) + str(sigm)
plt.xlabel(x_str)
plt.ylabel('$\\xi$', fontsize=15)
#plt.title('Decrease of cost over sigmoid of SDENNet')
plt.legend()
x1,x2,y1,y2 = plt.axis()
plt.axis((0,nb_of_iterations,0,6.0)) 
plt.grid()
plt.savefig("/home1/kzw/mlp/SDENNet_new_single_threshold/loss" + str(sigm) + ".png")
# plt.show()
